...
function showLoggedInApp(username) {
...
function logout() {
  // Save the current account's data before leaving.
  saveData();

  // Only remove the current session/account selection.
  // The account itself and its inventory/sales remain stored.
...
function setupAuthPage() {
  ...
  // IMPORTANT: keep the logged-in account in localStorage, not sessionStorage.
  // This means refreshing the page does not log the user out.
  const savedUser = localStorage.getItem(CURRENT_USER_KEY);
  if (savedUser) {
    const savedAccount = getAccounts().find(
      a => a.username.toLowerCase() === savedUser.toLowerCase()
    );

    if (savedAccount) {
      showLoggedInApp(savedAccount.username);
      return;
    }
  ...
  loginForm.addEventListener("submit", e => {
    ...
    // This is the only account selected as active.
    localStorage.setItem(CURRENT_USER_KEY, account.username);

    showLoggedInApp(account.username);
  });
...
function seedData() {
  // Every account starts completely empty.
  return { inv: [], sales: [] };
}

const CURRENT_USER_KEY = "shop-management-current-user";

function currentUsername() {
  return localStorage.getItem(CURRENT_USER_KEY) || "";
}

function accountDataKey(type, username = currentUsername()) {
  const normalized = username.trim().toLowerCase();
  const safeUsername = encodeURIComponent(normalized);
  return `shop-management-account-${safeUsername}-${type}`;
}
...
function saveData() {
  ...
  // Each account gets its own completely separate inventory and sales keys.
  localStorage.setItem(
    accountDataKey("inventory", username),
    JSON.stringify(inventory)
  );
  localStorage.setItem(
    accountDataKey("sales", username),
...
function loadData() {
  ...
  try {
    const savedInv = localStorage.getItem(accountDataKey("inventory", username));
    const savedSales = localStorage.getItem(accountDataKey("sales", username));

    if (savedInv !== null || savedSales !== null) {
      inventory = savedInv ? JSON.parse(savedInv) : [];
      sales = savedSales ? JSON.parse(savedSales) : [];
    } else {
      // First login for this account: create brand-new data ONLY for this account.
    ...
    <div class="modal-overlay" id="sale-overlay">
      <div class="modal" onclick="event.stopPropagation()">
        ...
        <form id="sale-form">
          <div class="field">
            <label>Product</label>
            <input name="product" id="sale-product" type="text" placeholder="Enter product name manually" autocomplete="off" required />
          </div>
