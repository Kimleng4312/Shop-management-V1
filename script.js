...
const LEGACY_INVENTORY_KEY = "shop-management-inventory";
const LEGACY_SALES_KEY = "shop-management-sales";
...
function accountDataKey(type, username = currentUsername()) {
  ...
  return `shop-management-${type}-${safeUsername}`;
}
...
function saveData() {
  ...
  localStorage.setItem(accountDataKey("inventory", username), JSON.stringify(inventory));
  localStorage.setItem(accountDataKey("sales", username), JSON.stringify(sales));
}
...
function loadData() {
  ...
  try {
    ...
    // Keep existing data for the original/default account on first login,
    // then permanently store it under that account.
    const legacyInv = localStorage.getItem(LEGACY_INVENTORY_KEY);
    const legacySales = localStorage.getItem(LEGACY_SALES_KEY);
    const seeded = seedData();

    if (savedInv || savedSales) {
      inventory = savedInv ? JSON.parse(savedInv) : [];
      sales = savedSales ? JSON.parse(savedSales) : [];
    } else if (username.toLowerCase() === "thina" && (legacyInv || legacySales)) {
      inventory = legacyInv ? JSON.parse(legacyInv) : seeded.inv;
      sales = legacySales ? JSON.parse(legacySales) : seeded.sales;
      saveData();
    } else {
